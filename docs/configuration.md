...
`qc` 由父进程启动**一个** spawn 服务进程，模型只加载一次。WSI worker 只做 CPU 预处理并把 batch 送进该服务；服务按 `batch_size` 把多个 worker 的请求拼成一次 GPU forward。`device: auto`（默认）在所有 GPU 上选空闲显存足够的卡；都不够则 warning 并回退 CPU。显式 `cuda:N` 时若该卡不存在或显存不够则报错。未列入 pipe 时不会启动服务、不加载模型。
